...
- после захвата apply-lock убирает прежний `client.vless`, затем подтверждает
  firewall и только после этого меняет сайт по SFTP; если E2E или выдача
  профиля завершается ошибкой либо прерывается и внешней конкурентной записи
  нет, возвращает transaction-owned `.htaccess` и выбранный `index.html` к
  точным pre-apply снимкам и не оставляет непроверенную клиентскую ссылку;
- перед rollback повторно сверяет exact SHA-256 установленного файла; если после
  переключения его успел изменить другой процесс, чужую правку не
  перезаписывает, сохраняет backup/quarantine с точным remote-путём в ошибке и
  явно сообщает о неполном rollback. SFTP не предоставляет CAS или
  `renameat2`, поэтому абсолютная атомарность с внешним writer между remote
  командами невозможна; проверяемые границы защищены quarantine-first и
  конфликт canonical target завершается fail-closed. Случайные имена
  `.xhttp-*` — transaction-owned namespace установщика; внешний процесс,
  намеренно меняющий такие cleanup-artifacts между их проверкой и следующей
  SFTP-командой, находится вне гарантии rollback;
- сохраняет краткую причину и статус rollback в `last-failure.log`, а при
  неуспешном E2E — ограниченный хвост Xray в
  `probe-runtime/var/lib/xhttp-setup/probe-failure.log`; оба файла имеют `0600`,
  UUID, VLESS Encryption, XHTTP path и полные URI удаляются из полного текста
  до ограничения длины и записи;
...
3. В ISPmanager создайте сайт с включённым PHP и выберите режим
   `FastCGI (Apache)`. На проверенной конфигурации REG.RU этот переключатель
   направляет отсутствующие пути через Apache, где выполняется `.htaccess`;
   сам PHP-код схема не запускает, поэтому выбранная версия PHP несущественна.
   Назначьте сайту этот FQDN и обязательно включите SSL для его vhost на 443.
   Для обычного режима выпустите Let's Encrypt; для pin-сценария можно выбрать
   выданный хостингом или самоподписанный сертификат. Снимать SHA-256 можно
   только после сохранения SSL-настроек сайта.
4. Для обычного режима проверьте `https://front.example.org/` браузером без
   предупреждений. Для pin-режима заранее получите SHA-256 сертификата, который
   endpoint реально отдаёт на клиентском IPv4 с указанным SNI. Проверьте, что по
   HTTPS открывается именно созданный сайт, а не служебная страница провайдера.
...
### Два режима TLS frontend

`public` используется по умолчанию: системный trust store проверяет цепочку CA,
срок и соответствие сертификата домену. В ссылке нет `pcs`.

`pinned` принимает ровно один SHA-256 текущего сертификата, сравнивает его с
первым (leaf) сертификатом, реально полученным с `client-connect-ip:443`, и
только затем создаёт клиентский JSON с `pinnedPeerCertSha256` и URI с `pcs=`.
SNI и XHTTP Host всё равно остаются равны frontend-домену. В этом режиме
обычная PKI-проверка CA, SAN/hostname, срока и revocation не выполняется:
доверие основано исключительно на совпадении exact leaf SHA-256. Поэтому это не
эквивалент публичного TLS. При этом простого «принять любой сертификат» тоже нет:
безусловный `allowInsecure` не используется, другой leaf будет отвергнут.

Pin не является постоянным: после перевыпуска или замены сертификата соединение
перестанет работать до повторной проверки, обновления pin и перевыдачи ссылки.
Автоматического обновления pin нет.
Нельзя закреплять root/intermediate CA вместо фактически предъявленного leaf.
Клиентское приложение должно понимать `pinnedPeerCertSha256`/`pcs`; импорт URI
сам по себе это не гарантирует.

Pin обязательно снимается с TLS-handshake с SNI frontend-домена. Вывод
`xray tls ping` содержит отдельные секции `Pinging without SNI` и
`Pinging with SNI`: hash из первой секции использовать запрещено. На одном IP
без SNI провайдер может предъявить wildcard/default-vhost сертификат, а с SNI —
совсем другой сертификат сайта. Если секция с SNI не печатает сертификат из-за
ошибки обычной PKI-проверки, это не повод подставлять hash из `without SNI`;
используйте способ снятия leaf DER, который явно задаёт SNI.

`pcs` проверяет сертификат, но не назначает Apache vhost. Если SSL для сайта в
ISPmanager не включён и порт 443 попадает на default-vhost с сообщением вроде
«домен не привязан», закрепление сертификата default-vhost только зафиксирует
неправильный endpoint. Сначала включается SSL/443 именно у сайта, затем
проверяются SNI/Host и содержимое сайта, и только после этого снимается leaf pin.

Поля формы ISPmanager, отключаемые опции и точный порядок для shared-hosting
вынесены в [`docs/reg-ru-pinned-tls.txt`](docs/reg-ru-pinned-tls.txt).
...
- автоматический rollback покрывает неуспешный запуск managed systemd-сервиса и
  ...
  отдельной команды rollback после уже успешной установки в 0.1.0 ещё нет;
