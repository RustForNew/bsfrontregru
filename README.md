...
сайт. В 0.1.0 сайт, SSL-vhost и сертификат создаются заранее через ISPmanager.
...
Мастер не включает UFW и не меняет default policy. Канонический runbook сначала
сохраняет доступ на фактическом SSH-порту и закрывает backend, а уже затем
запускает Xray. После `exit` создаётся:
...
- внутренние API `remote_exit` и `exit_network` покрыты unit-тестами, но пока не
  подключены к основному меню и отдельно live не проверялись. MSS в optional-
  профиле остаётся runtime-only и не обещает persistence после reboot.
